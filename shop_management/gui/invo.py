...
class InvoiceTab(ctk.CTkFrame):
    ...
    button_frame,
    text="Create Invoice",
    command=self.create_invoice).grid(
        row=0,
        column=0,
         padx=5)
        ctk.CTkButton(
    button_frame,
    text="Refresh Lists",
    command=self.refresh_invoice_list).grid(
        row=0,
        column=1,
         padx=5)
        ctk.CTkButton(
    ...
    self, text="Invoices", font=ctk.CTkFont(
        size=16, weight="bold"))
    ...
    self, columns=invoice_columns, show="headings", height=7)
    ...
    self, orient="vertical", command=self.invoice_table.yview)
...
pending_label = ctk.CTkLabel(
    self,
    text="Pending Sales for Invoice",
    font=ctk.CTkFont(
        size=16,
         weight="bold"))
...
self.pending_sales_table = ttk.Treeview(
    self, columns=pending_columns, show="headings", height=6)
...
pending_scroll = ttk.Scrollbar(
    self,
    orient="vertical",
     command=self.pending_sales_table.yview)
...
pending_scroll.pack(side="right", fill="y")
   ...
   def create_invoice(self):
       ...
       cursor.execute(
    'SELECT invoice_id FROM invoices WHERE sale_id=?', (sale_id,))
       if cursor.fetchone():
            messagebox.showinfo(
    "Info", f"Invoice already exists for Sale ID {sale_id}")
           conn.close()
           return
       ...
       messagebox.showinfo(
    "Success",
     f"Invoice for Sale ID {sale_id} created")
       self.refresh_invoice_list()
       self.refresh_sales_list()
    ...
    def refresh_sales_list(self):
        ...
        for s in sales:
            line = f"{s[0]:<5}{s[1]:<20}{s[2]:<10}{s[3]:<5}{s[4]:<10.2f}{s[5]}\n"
